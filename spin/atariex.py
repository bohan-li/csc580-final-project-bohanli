...
flags.DEFINE_integer(
    'neig', 10, 'Number of Eigen values to compute. Must be greater than 1.')
flags.DEFINE_integer('niter', 1, 'Number of iterations.')
flags.DEFINE_integer('batch_size', 32, 'Self-explanatory.')
...
flags.DEFINE_string('game', 'space_invaders',
...
_NFRAMES = 263
_IMAGE_SIZE = 80  # Image side length.
_KERNEL_SIZE = 6
...
def train(iterations,
      ...
      for i in range(2):
          for j in range(neig):
              ...
              frame_im.append(frame_ax[i, j].imshow(
                  ...
                  interpolation='none',
                  cmap='plasma', vmin=0.0, vmax=255.0))
  ...
  image = np.fromfile('meansubtracted.txt').astype(np.float32)
  image = np.reshape(image, (_NFRAMES, _IMAGE_SIZE, _IMAGE_SIZE))

  batch = np.zeros((batch_size + 1, _IMAGE_SIZE, _IMAGE_SIZE, 4),
                   dtype=np.float32)

  def _reader():
    idx = np.random.randint(0, _NFRAMES - batch_size - 4)
    for i in range(batch_size+1):
      batch[i] = image[idx+i:idx+i+4].transpose((1, 2, 0))
    return batch
